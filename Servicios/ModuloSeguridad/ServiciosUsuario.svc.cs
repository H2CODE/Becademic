...
namespace Servicios.ModuloSeguridad
{
    ...
    public class ServiciosUsuario
    {
        ...
        [OperationContract]
        [WebGet(UriTemplate = "/cedulaunica?fieldId={llave}&fieldValue={valor}", ResponseFormat = WebMessageFormat.Json)]
        public Array CedulaUnica(object llave, int valor)
        {
            IEnumerable<Usuario> lstUsuario = gestorUsuario.consultarUsuario();

            bool noExiste = true;
            foreach(Usuario usr in lstUsuario){
                if (usr.Cedula == valor)
                    noExiste = false;
            }

            return new Object[]{llave.ToString(), noExiste};
        }

        [OperationContract]
        [WebGet(UriTemplate = "/correounico?fieldId={llave}&fieldValue={valor}", ResponseFormat = WebMessageFormat.Json)]
        public Array CorreoUnico(object llave, string valor)
        {
            IEnumerable<Usuario> lstUsuario = gestorUsuario.consultarUsuario();
